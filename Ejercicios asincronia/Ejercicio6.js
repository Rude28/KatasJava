...
const sum = exams.reduce((acc, currentExam) => acc + currentExam.score, 0);
...
const sumApproved = exams.reduce((acc, currentExam) => {

    if (currentExam.score >= 5) {
        return acc + currentExam.score;
...
const average = exams.reduce((acc, currentExam, i, array) => {
    acc += currentExam.score;
